...
TARGET_WIDTH = 256  # or any custom width you desire
TARGET_HEIGHT = 256  # or any custom height you desire
...
def main():
    ...
    #val_paths, val_labels = read_label_file(os.path.join(LABELS_DIR, 'val.txt'))
    ...
    """ for idx, path in enumerate(val_paths):
        tasks.append((path, 'val.txt', idx + len(train_paths) + len(test_paths), total_images)) """
    ...
    with h5py.File('dataset.h5', 'w') as hf:
        ...
        """ train_images_dset = hf.create_dataset("train_images", (TEST_LIMIT, TARGET_HEIGHT, TARGET_WIDTH), dtype='uint8')
        train_labels_dset = hf.create_dataset("train_labels", (TEST_LIMIT,), dtype='int32')
        test_images_dset = hf.create_dataset("test_images", (TEST_LIMIT, TARGET_HEIGHT, TARGET_WIDTH), dtype='uint8')
        test_labels_dset = hf.create_dataset("test_labels", (TEST_LIMIT,), dtype='int32') """
        """ val_images_dset = hf.create_dataset("val_images", (TEST_LIMIT, TARGET_HEIGHT, TARGET_WIDTH), dtype='uint8')
        val_labels_dset = hf.create_dataset("val_labels", (TEST_LIMIT,), dtype='int32') """
            ...
            with Pool(cpu_count()) as pool:
                ...
                for idx, (img, label_path) in enumerate(pool.imap_unordered(worker_wrapper, tasks_with_counter)):
                    ...
                    if "train" in label_path:
                        ...
                        testCount+=1
                    """ elif "val" in label_path:
                        val_images_dset[valCount] = img
                        val_labels_dset[valCount] = val_labels[valCount]
                        valCount+=1 """
