...
def findImageInData(imageDirec):
    imageDirecParts = imageDirec.split('/')
    #print(imageDirecParts[-1])

    with open("Image Database/labels/test.txt") as file:
        for line in file:
            if imageDirecParts[-1] in line:
                return line.rstrip()[-1], 0

    with open("Image Database/labels/train.txt") as file:
        for line in file:
            if imageDirecParts[-1] in line:
...
def searchFiles():
    ...
    for subdir, dirs, files in os.walk('Image Database/images'):
        for file in files:
            ...
            image = skimage.io.imread(fileDir)
            new_image_resized = skimage.transform.resize(image, (500, 500, 3))
            new_image = skimage.color.rgb2gray(new_image_resized)
            b_and_w = skimage.filters.threshold_otsu(new_image)
            new_image = new_image > b_and_w
            ...
            match dataType:
                case 0:
                    skimage.io.imsave((os.path.join(pathTotestingFolder, ImageName)), new_image)
                    testData.write(str(Newline) + "\n")
                case 1:
                    skimage.io.imsave((os.path.join(pathTotrainingFolder, ImageName)), new_image)
                    trainData.write(str(Newline) + "\n")
                case 2:
                    skimage.io.imsave((os.path.join(pathTovalidationFolder, ImageName)), new_image)
